		...
		if (is_dir($languagePath)) {
			...
			foreach ($languageFiles as $languageFile) {
				...
				if (preg_match('/^(-?[a-z]+){1,7}\.json$/i', $languageFile) === 0) {
					$this->h5pF->setErrorMessage($this->h5pF->t('Invalid language file %file in library %library', array(
						'%file' => $languageFile,
						'%library' => $file
					)));
				...
				if ($languageJson === false) {
					$this->h5pF->setErrorMessage($this->h5pF->t('Invalid language file %languageFile has been included in the library %name', array(
	...
	private function isExistingFiles($files, $tmpDir, $library) {
		foreach ($files as $file) {
			...
			if (!file_exists($tmpDir . DIRECTORY_SEPARATOR . $library . DIRECTORY_SEPARATOR . $path)) {
				$this->h5pF->setErrorMessage($this->h5pF->t('The file "%file" is missing from library: "%name"', array(
					'%file' => $path,
					'%name' => $library
				)));
	...
	private function isValidRequirement($h5pData, $requirement, $library_name, $property_name) {
		...
		if (is_string($requirement)) {
			if ($requirement == 'boolean') {
				if (!is_bool($h5pData)) {
					$this->h5pF->setErrorMessage($this->h5pF->t("Invalid data provided for %property in %library. Boolean expected.", array(
				...
				if (is_string($h5pData) || is_int($h5pData)) {
					if (preg_match($requirement, $h5pData) === 0) {
						$this->h5pF->setErrorMessage($this->h5pF->t("Invalid data provided for %property in %library", array(
							'%property' => $property_name,
							'%library' => $library_name
						)));
						$valid = false;
			...
			$this->h5pF->setErrorMessage($this->h5pF->t("Can't read the property %property in %library", array(
				'%property' => $property_name,
				'%library' => $library_name
			)));
			$valid = false;
	...
	private function isValidRequiredH5pData($h5pData, $requirements, $library_name) {
		...
		foreach ($requirements as $required => $requirement) {
			...
			if (isset($h5pData[$required])) {
				...
				$this->h5pF->setErrorMessage($this->h5pF->t('The required property %property is missing from %library', array(
					'%property' => $required,
					'%library' => $library_name
				)));
				$valid = false;
	...
	private function isValidH5pDataOptions($selected, $allowed, $library_name) {
		...
		foreach ($selected as $value) {
			if (!in_array($value, $allowed)) {
				$this->h5pF->setErrorMessage($this->h5pF->t('Illegal option %option in %library', array(
					'%option' => $value,
					'%library' => $library_name
				)));
				$valid = false;
...
class H5PCore {
	...
	public function filterParameters(&$content) {
		if (!empty($content['filtered'])
			&& (!$this->exportEnabled || ($content['slug'] && $this->fs->hasExport($content['slug'] . '-' . $content['id'] . '.h5p')))) {
		...
		foreach (array( 'dynamic', 'preloaded', 'editor' ) as $type) {
			...
			foreach ($library[$property] as $dependency) {
				...
				if ($dependencyLibrary) {
					...
					$this->h5pF->setErrorMessage($this->h5pF->t('Missing dependency @dep required by @lib.', array(
	...
	public static function libraryToString($library, $folderName = false) {
		return (isset($library['machineName']) ? $library['machineName'] : $library['name']) . ($folderName ? '-' : ' ') . $library['majorVersion']
	...
	public function getUpgrades($library, $versions) {
		...
		foreach ($versions as $upgrade) {
			if ($upgrade->major_version > $library->major_version
				|| $upgrade->major_version === $library->major_version
	...
	public function detectSiteType() {
		...
		if ($type === 'network'
			|| ($type === 'local' && isset($_SERVER['REMOTE_ADDR'])
	...
	public function fetchLibrariesMetadata($fetchingDisabled = false) {
		...
		if (empty($uuid)) {
			...
			if (!$registration) {
				...
				$this->h5pF->setErrorMessage($this->h5pF->t('The H5P Hub has been disabled until this problem can be resolved. You may still upload libraries through the "H5P Libraries" page.'));
			...
			$this->h5pF->setInfoMessage($this->h5pF->t('Your site was successfully registered with the H5P Hub.'));
		...
		if ($this->h5pF->getOption('send_usage_statistics', true)) {
			$siteData = array_merge($registrationData, array(
	...
	public function getStorableDisplayOptions(&$sources, $current) {
		...
		if ($download == H5PDisplayOptionBehaviour::ALWAYS_SHOW || $download == H5PDisplayOptionBehaviour::NEVER_SHOW) {
		...
		if ($embed == H5PDisplayOptionBehaviour::ALWAYS_SHOW || $embed == H5PDisplayOptionBehaviour::NEVER_SHOW) {
	...
	public function getDisplayOptionsForEdit($disable = NULL) {
		...
		if ($this->h5pF->getOption(self::DISPLAY_OPTION_FRAME, true)) {
			$display_options[self::DISPLAY_OPTION_FRAME] = isset($current_display_options[self::DISPLAY_OPTION_FRAME]) ? $current_display_options[self::DISPLAY_OPTION_FRAME] : true;
			...
			if ($export == H5PDisplayOptionBehaviour::CONTROLLED_BY_AUTHOR_DEFAULT_ON
				...
				$display_options[self::DISPLAY_OPTION_DOWNLOAD] = isset($current_display_options[self::DISPLAY_OPTION_DOWNLOAD]) ? $current_display_options[self::DISPLAY_OPTION_DOWNLOAD] : ($export
			...
			if ($embed == H5PDisplayOptionBehaviour::CONTROLLED_BY_AUTHOR_DEFAULT_ON
				...
				$display_options[self::DISPLAY_OPTION_EMBED] = isset($current_display_options[self::DISPLAY_OPTION_EMBED]) ? $current_display_options[self::DISPLAY_OPTION_EMBED] : ($embed
					== H5PDisplayOptionBehaviour::CONTROLLED_BY_AUTHOR_DEFAULT_ON);
			}

			// Copyright
			if ($this->h5pF->getOption(self::DISPLAY_OPTION_COPYRIGHT, true)) {
				$display_options[self::DISPLAY_OPTION_COPYRIGHT] = isset($current_display_options[self::DISPLAY_OPTION_COPYRIGHT]) ? $current_display_options[self::DISPLAY_OPTION_COPYRIGHT] : true;
			}
		}
	...
	public static function validToken($action, $token) {
		...
		return $token === self::hashToken($action, $time_factor)
			|| // Under 12 hours
	...
	public function updateContentTypeCache($postData = NULL) {
		...
		// No data received
		if (!$data) {
			$interface->setErrorMessage($interface->t("Couldn't communicate with the H5P Hub. Please try again later."));
		...
		if (!isset($json->contentTypes) || empty($json->contentTypes)) {
			$interface->setErrorMessage($interface->t('No content types were received from the H5P Hub. Please try again later.'));
	...
	public function checkSetupErrorMessage() {
		...
		if (!extension_loaded('mbstring')) {
			$setup->errors[] = $this->h5pF->t('The mbstring PHP extension is not loaded. H5P needs this to function properly');
			$setup->disable_hub = true;
		}
		...
		if ($max_upload_size < $byte_threshold) {
			$setup->errors[] = $this->h5pF->t('Your PHP max upload size is quite small. With your current setup, you may not upload files larger than %number MB. This might be a problem when trying to upload H5Ps, images and videos. Please consider to increase it to more than 5MB.', array(
				'%number' => number_format($max_upload_size / 1024 / 1024, 2, '.', ' ')
			));
		}

		if ($max_post_size < $byte_threshold) {
			$setup->errors[] = $this->h5pF->t('Your PHP max post size is quite small. With your current setup, you may not upload files larger than %number MB. This might be a problem when trying to upload H5Ps, images and videos. Please consider to increase it to more than 5MB', array(
				'%number' => number_format($max_upload_size / 1024 / 1024, 2, '.', ' ')
			));
		}

		if ($max_upload_size > $max_post_size) {
			$setup->errors[] = $this->h5pF->t('Your PHP max upload size is bigger than your max post size. This is known to cause issues in some installations.');
		}
	...
	public function checkSetupForRequirements() {
		...
		if ($setup->disable_hub) {
			...
			$this->h5pF->setErrorMessage($this->h5pF->t('H5P hub communication has been disabled because one or more H5P requirements failed.'));
			$this->h5pF->setErrorMessage($this->h5pF->t('When you have revised your server setup you may re-enable H5P hub communication in H5P Settings.'));
		}
	}
...
class H5PContentValidator {
	...
	public function validateText(&$text, $semantics) {
		...
		if (!($text === '' && isset($semantics->optional) && $semantics->optional) && isset($semantics->regexp)) {
			...
			if (preg_match($pattern, $text) === 0) {
				...
				$this->h5pF->setErrorMessage($this->h5pF->t('Provided string is not valid according to regexp in semantics. (value: "%value", regexp: "%regexp")', array(
					'%value' => $text,
					'%regexp' => $pattern
				)));
				$text = '';
			...
			if (is_dir($filePath)) {
				...
				if (!extension_loaded('mbstring')) {
					...
					if (!preg_match($wl_regex, mb_strtolower($file))) {
						$this->h5pF->setErrorMessage($this->h5pF->t('File "%filename" not allowed. Only files with the following extensions are allowed: %files-allowed.', array(
							'%filename' => $file,
	 ...
	 * @ingroup sanitation
	 */
	private function filter_xss($string, $allowed_tags = array(
		'a',
		'em',
		'strong',
		'cite',
