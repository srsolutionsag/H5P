...
class H5PShowContent {
	 ...
	 * @return string
	 */
	public function getH5PContentIntegration(H5PContent $h5p_content, $title = true) {
	...
	protected function getContent(H5PContent $h5p_content) {
		...
		$content_user_datas = H5PContentUserData::getUserDatasByUser($user_id, $h5p_content->getContentId());
	...
	public function setFinished($content_id, $score, $max_score, $opened, $finished, $time = NULL) {
		...
		$h5p_result = H5PResult::getResultByUserContent($user_id, $content_id);

		$new = false;
		if ($h5p_result === NULL) {
			$h5p_result = new H5PResult();
		...
		if ($object !== NULL) {
			...
			H5PSolveStatus::setContentByUser($h5p_content->getObjId(), $user_id, $h5p_content->getContentId());
		}
	}
	...
	public function contentsUserData($content_id, $data_id, $sub_content_id, $data = NULL, $preload = false, $invalidate = false) {
		...
		$h5p_content_user_data = H5PContentUserData::getUserData($content_id, $data_id, $user_id, $sub_content_id);
		...
		if ($data !== NULL) {
			if ($data === "0") {
				...
				$new = false;
				if ($h5p_content_user_data === NULL) {
					$h5p_content_user_data = new H5PContentUserData();
					...
					if ($object !== NULL && $object->isSolveOnlyOnce() && H5PResult::hasContentResults($h5p_content->getContentId())) {
						die();
					}
				}
