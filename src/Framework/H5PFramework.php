...
class H5PFramework implements H5PFrameworkInterface {
	...
	public function setLibraryTutorialUrl($machine_name, $tutorial_url) {
		$h5p_libraries = H5PLibrary::getLibraryAllVersions($machine_name);
	...
	public function loadLibraries() {
		$h5p_libraries = H5PLibrary::getLibraries();
	...
	public function getLibraryId($machine_name, $major_version = NULL, $minor_version = NULL) {
		$h5p_library = H5PLibrary::getLibraryByVersion($machine_name, $major_version, $minor_version);
	...
	public function isPatchedLibrary($library) {
		...
		$h5p_library = H5PLibrary::getLibraryByVersion($library["machineName"], $library["majorVersion"], $library["minorVersion"]);
	...
	public function saveLibraryData(&$library_data, $new = true) {
		if ($new) {
			...
			$h5p_library = H5PLibrary::getLibraryById($library_data["libraryId"]);
		...
		$h5p_event = new H5PEventFramework("library", ($new ? "create" : "update"), NULL, NULL, $h5p_library->getName(), ($h5p_library->getMajorVersion()
			. "." . $h5p_library->getMinorVersion()));

		$h5p_languages = H5PLibraryLanguage::getLanguagesByLibrary($h5p_library->getLibraryId());
		...
		if (isset($library_data["language"])) {
			foreach ($library_data["language"] as $language_code => $language_json) {
				$h5p_language = new H5PLibraryLanguage();
	...
	public function updateContent($content, $content_main_id = NULL) {
		...
		$h5p_event = new H5PEventFramework("content", (($new ? "create" : "update")
	...
	public function resetContentUserData($content_id) {
		$h5p_user_datas = H5PContentUserData::getUserDatasByContent($content_id);
	...
	public function saveLibraryDependencies($library_id, $dependencies, $dependency_type) {
		foreach ($dependencies as $dependency) {
			$h5p_library = H5PLibrary::getLibraryByVersion($dependency["machineName"], $dependency["majorVersion"], $dependency["minorVersion"]);

			$h5p_dependency = new H5PLibraryDependencies();
	...
	public function copyLibraryUsage($content_id, $copy_from_id, $content_main_id = NULL) {
		$h5p_content_libraries = H5PContentLibrary::getContentLibraries($copy_from_id);

		foreach ($h5p_content_libraries as $h5p_content_library) {
			/**
			 * @var H5PContentLibrary $h5p_content_library_copy
			 */
	...
	public function deleteContentData($content_id) {
		...
		$h5p_event = new H5PEventFramework("content", "delete", $content_id, $content["title"], $content["libraryName"], ($content["libraryMajorVersion"]
			. "." . $content["libraryMinorVersion"]));

		$h5p_content = H5PContent::getContentById($content_id);
		if ($h5p_content !== NULL) {
			$h5p_content->delete();
		}
		...
		$h5p_results = H5PResult::getResultsByContent($content_id);
	...
	public function deleteLibraryUsage($content_id) {
		$h5p_content_libraries = H5PContentLibrary::getContentLibraries($content_id);
	...
	public function saveLibraryUsage($content_id, $libraries_in_use) {
		...
		foreach ($libraries_in_use as $library_in_use) {
			$h5p_content_library = new H5PContentLibrary();
	...
	public function getLibraryUsage($library_id, $skip_content = false) {
		if (!$skip_content) {
			$content = H5PLibrary::getLibraryUsage($library_id);
		} else {
			$content = - 1;
		}

		$libraries = H5PLibraryDependencies::getLibraryUsage($library_id);
	...
	public function loadLibrary($machine_name, $major_version, $minor_version) {
		...
		if ($h5p_library !== NULL) {
			...
			$h5p_dependencies = H5PLibraryDependencies::getDependenciesJoin($h5p_library->getLibraryId());
	...
	public function deleteLibraryDependencies($library_id) {
		$h5p_dependencies = H5PLibraryDependencies::getDependencies($library_id);
	...
	public function deleteLibrary($library) {
		...
		$h5p_languages = H5PLibraryLanguage::getLanguagesByLibrary($library->library_id);
		foreach ($h5p_languages as $h5p_language) {
			$h5p_language->delete();
		}

		$h5p_library = H5PLibrary::getLibraryById($library->library_id);
		if ($h5p_library !== NULL) {
			$h5p_library->delete();
		}

		$h5p_event = new H5PEventFramework("library", "delete", NULL, NULL, $h5p_library->getName(), ($h5p_library->getMajorVersion() . "."
			. $h5p_library->getMinorVersion()));
	}
	...
	public function loadContent($id) {
		...
		$h5p_content = H5PContent::getContentById($id);
		...
		if ($h5p_content !== NULL) {
			...
			$h5p_library = H5PLibrary::getLibraryById($h5p_content->getLibraryId());
			if ($h5p_library !== NULL) {
	...
	public function loadContentDependencies($id, $type = NULL) {
		...
		$h5p_content_libraries = H5PContentLibrary::getContentLibraries($id, $type);

		foreach ($h5p_content_libraries as $h5p_content_library) {
			$h5p_library = H5PLibrary::getLibraryById($h5p_content_library->getLibraryId());
	...
	public function getOption($name, $default = NULL) {
		return H5POption::getOption($name, $default);
	}
	...
	public function setOption($name, $value) {
		H5POption::setOption($name, $value);
	}
	...
	public function clearFilteredParameters($library_id) {
		$h5p_contents = H5PContent::getContentsByLibrary($library_id);
	...
	public function isContentSlugAvailable($slug) {
		$h5p_content = H5PContent::getContentsBySlug($slug);
	...
	public function getLibraryStats($type) {
		$h5p_counters = H5PCounter::getCountersByType($type);
	...
	public function getNumAuthors() {
		return H5PContent::getNumAuthors();
	}
	...
	public function saveCachedAssets($key, $libraries) {
		foreach ($libraries as $library) {
			$h5p_cached_asset = new H5PLibraryCachedAsset();
	...
	public function deleteCachedAssets($library_id) {
		$h5p_cached_assets = H5PLibraryCachedAsset::getCachedAssetsByLibrary($library_id);
	...
	public function getLibraryContentCount() {
		...
		foreach ($h5p_libraries as $h5p_library) {
			...
			. $h5p_library->getMinorVersion()] = count(H5PContent::getContentsByLibrary($h5p_library->getLibraryId()));
		}
	...
	public function replaceContentTypeCache($content_type_cache) {
		H5PLibraryHubCache::truncateDB();

		foreach ($content_type_cache->contentTypes as $content_type) {
			$library_hub_cache = new H5PLibraryHubCache();
